...
const T = {
  tr: {
    ...
    allPosts:"Tüm İlanlar", myPosts:"İlanlarım", search:"İlan ara...",
    internships:"Staj İlanları 💼", internshipSub:"Bilkent öğrencilerine özel fırsatlar",
    apply:"Başvur →", postInternship:"+ İlan Ver", discover:"Keşfet 🔍",
    discoverSub:"Kampüste neler oluyor?", popular:"🔥 En Popüler",
    profile:"Profil", settings:"Ayarlar ⚙️", messages:"Mesajlar 💬",
    logout:"Çıkış Yap", save:"Kaydet", cancel:"İptal", edit:"Düzenle",
    login:"Giriş Yap", register:"Kayıt Ol", forgotPass:"Şifremi unuttum",
    darkMode:"Koyu Tema", language:"Dil", favorites:"Favoriler ❤️",
    noFav:"Henüz favori eklemedin.", send:"Gönder", newMsg:"Yeni Mesaj",
    to:"Alıcı (e-posta)", msgContent:"Mesajınız...", noMsg:"Henüz mesaj yok.",
    companyAccount:"Şirket Hesabı", studentAccount:"Öğrenci Hesabı",
    accountType:"Hesap Türü", companyName:"Şirket Adı",
  },
  en: {
    hello:"Hello", looking:"What are you looking for today?", newPost:"+ Post",
    allPosts:"All Posts", myPosts:"My Posts", search:"Search...",
    internships:"Internships 💼", internshipSub:"Exclusive opportunities for Bilkent students",
    apply:"Apply →", postInternship:"+ Post", discover:"Discover 🔍",
    discoverSub:"What's happening on campus?", popular:"🔥 Most Popular",
    profile:"Profile", settings:"Settings ⚙️", messages:"Messages 💬",
    logout:"Logout", save:"Save", cancel:"Cancel", edit:"Edit",
    login:"Login", register:"Sign Up", forgotPass:"Forgot password",
    darkMode:"Dark Mode", language:"Language", favorites:"Favorites ❤️",
    noFav:"No favorites yet.", send:"Send", newMsg:"New Message",
    to:"Recipient (email)", msgContent:"Your message...", noMsg:"No messages yet.",
    companyAccount:"Company Account", studentAccount:"Student Account",
    accountType:"Account Type", companyName:"Company Name",
  }
}

function getCSS(dark) {
  const bg     = dark ? "#0f0f13" : "#f5f4f0"
  const card   = dark ? "#1a1a24" : "#ffffff"
  const border = dark ? "#2a2a38" : "#e8e6e0"
  const text   = dark ? "#e8e6f0" : "#1a1a2e"
  const sub    = dark ? "#6b7280" : "#9ca3af"
  const inp    = dark ? "#13131a" : "#f9f8f6"
...
.nav-item.active{background:linear-gradient(135deg,#ede9fe22,#ddd6fe22);color:#6366f1;font-weight:600;}
...
.card:hover{border-color:#c4b5fd44;box-shadow:0 4px 20px rgba(99,102,241,.08);transform:translateY(-1px);}
...
.catbtn.on{background:linear-gradient(135deg,#ede9fe22,#ddd6fe44);border-color:#c4b5fd;color:#6366f1;font-weight:600;}
...
function Auth({ onLogin }) {
  ...
  const [accType,setAccType]=useState("ogrenci")
  ...
  async function login() {
    ...
    if(!profile){
      const at=data.user.user_metadata?.account_type||"ogrenci"
      await supabase.from("profiles").insert({id:data.user.id,name:data.user.user_metadata?.name||email.split("@")[0],email,account_type:at})
    }
    onLogin({
      id:data.user.id,
      ...
      account_type:profile?.account_type||data.user.user_metadata?.account_type||"ogrenci"
    })
  }
              ...
              <div>
                ...
                <div style={{display:"flex",gap:10}}>
                  {[["ogrenci","🎓 Öğrenci"],["sirket","🏢 Şirket"]].map(([k,l])=>(
                    <button key={k} onClick={()=>setAccType(k)} style={{flex:1,padding:"10px",borderRadius:12,border:`2px solid ${accType===k?"#6366f1":"#e8e6e0"}`,background:accType===k?"#ede9fe":"#fff",color:accType===k?"#6366f1":"#6b7280",cursor:"pointer",fontFamily:"'Plus Jakarta Sans'",fontWeight:600,fontSize:13}}>
                      {l}
                    </button>
                  ))}
                </div>
...
// ── NAV ───────────────────────────────────────────────────────────────────────
function Sidebar({active,setActive,user,onLogout,dark,unread,t}) {
  const NAV=[
    {key:"feed",icon:"🏠",label:t.allPosts},
    {key:"staj",icon:"💼",label:"Staj"},
    {key:"kesfet",icon:"🔍",label:"Keşfet"},
    {key:"messages",icon:"💬",label:t.messages,badge:unread},
  ...
  return (
    <div className="sidebar">
      ...
      <div style={{borderTop:"1px solid var(--border,#e8e6e0)",paddingTop:16}}>
        <div style={{display:"flex",alignItems:"center",gap:10,padding:"8px 12px",marginBottom:8}}>
          <Avatar name={user.name} src={user.avatar} size={32}/>
          <div style={{flex:1,minWidth:0}}>
            <div style={{fontSize:13,fontWeight:600,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{user.name}</div>
            <div style={{fontSize:10,color:"#9ca3af",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{user.account_type==="sirket"?"🏢 Şirket":"🎓 Öğrenci"}</div>
          </div>
        </div>
...
function BottomNav({active,setActive,unread}) {
  const NAV=[
    {key:"feed",icon:"🏠"},{key:"staj",icon:"💼"},{key:"kesfet",icon:"🔍"},
...
function FeedPage({user,t}) {
  ...
  async function toggleFav(post) {
    const favs=post.favorited_by||[]
    const nf=favs.includes(user.email)?favs.filter(e=>e!==user.email):[...favs,user.email]
  ...
  async function addPost() {
    ...
    setPosting(true)
    const {data,error}=await supabase.from("posts").insert({user_id:user.id,user_name:user.name,user_email:user.email,category:form.category,title:form.title,description:form.desc,likes:[],favorited_by:[]}).select().single()
  ...
  return (
    <div className="page">
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:24}}>
        <div>
          <div style={{fontFamily:"'Sora',sans-serif",fontWeight:700,fontSize:22}}>{t.hello}, {user.name.split(" ")[0]} 👋</div>
          <div style={{fontSize:13,color:"#9ca3af",marginTop:2}}>{t.looking}</div>
        </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10,marginBottom:24}}>
        {["ev","esya","ders"].map(k=>(
          <div key={k} onClick={()=>setFilter(k)} style={{background:"var(--card,#fff)",border:`2px solid ${filter===k?"#6366f1":"var(--border,#e8e6e0)"}`,borderRadius:14,padding:"14px 10px",textAlign:"center",cursor:"pointer",transition:"all .2s"}}>
      ...
      <div style={{display:"flex",gap:20,borderBottom:"1px solid var(--border,#e8e6e0)",marginBottom:16}}>
...
function StajPage({user,t}) {
  ...
  return (
    <div className="page">
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:24}}>
        <div>
          <div style={{fontFamily:"'Sora',sans-serif",fontWeight:700,fontSize:22}}>{t.internships}</div>
          <div style={{fontSize:13,color:"#9ca3af",marginTop:2}}>{t.internshipSub}</div>
        </div>
        {isSirket&&<button className="sbtn" onClick={()=>setModal(true)}>{t.postInternship}</button>}
      </div>

      {!isSirket&&(
        <div style={{background:"#ede9fe",border:"1px solid #c4b5fd",borderRadius:14,padding:"12px 16px",fontSize:13,color:"#6366f1",marginBottom:20,fontWeight:500}}>
          💡 Staj ilanı vermek için şirket hesabı gereklidir.
        </div>
      )}
  ...
  return (
    <div className="page">
      <div style={{marginBottom:24}}>
        <div style={{fontFamily:"'Sora',sans-serif",fontWeight:700,fontSize:22}}>{t.discover}</div>
        <div style={{fontSize:13,color:"#9ca3af",marginTop:2}}>{t.discoverSub}</div>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:10,marginBottom:28}}>
        {Object.entries(CAT).map(([k,v])=>(
          <div key={k} style={{background:"var(--card,#fff)",border:"1px solid var(--border,#e8e6e0)",borderRadius:16,padding:16,display:"flex",alignItems:"center",gap:12}}>
      ...
      <div style={{fontFamily:"'Sora',sans-serif",fontWeight:700,fontSize:16,marginBottom:14}}>{t.popular}</div>
      {loading?(
...
// ── MESSAGES PAGE ─────────────────────────────────────────────────────────────
function MessagesPage({user,t,onRead}) {
  ...
  async function loadConvos() {
    ...
    const totalUnread=Object.values(map).reduce((a,c)=>a+c.unread,0)
    onRead(totalUnread)
    setLoading(false)
  }
  ...
  async function startConvo() {
    if(!toEmail.trim()||!firstMsg.trim()) return
    setSending(true)
    await supabase.from("messages").insert({from_email:user.email,from_name:user.name,to_email:toEmail,to_name:toName||toEmail,content:firstMsg})
    setModal(false);setToEmail("");setToName("");setFirstMsg("")
    await loadConvos()
    setActive(toEmail)
    setSending(false)
  }
  ...
  return (
    <div className="page">
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:24}}>
        <div style={{fontFamily:"'Sora',sans-serif",fontWeight:700,fontSize:22}}>{t.messages}</div>
        <button className="sbtn" onClick={()=>setModal(true)}>{t.newMsg}</button>
      </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:active?"1fr 1.5fr":"1fr",gap:16}}>
        <div style={{display:"flex",flexDirection:"column",gap:8}}>
          {loading?(
            ...
            <div style={{textAlign:"center",padding:40,color:"#9ca3af"}}>
              ...
              <div style={{fontSize:13}}>{t.noMsg}</div>
            </div>
          ):convos.map(c=>(
            <div key={c.email} onClick={()=>setActive(c.email)} style={{background:"var(--card,#fff)",border:`1px solid ${active===c.email?"#6366f1":"var(--border,#e8e6e0)"}`,borderRadius:14,padding:"14px 16px",cursor:"pointer",display:"flex",alignItems:"center",gap:12,transition:"all .2s"}}>
              <Avatar name={c.name} size={40}/>
              <div style={{flex:1,minWidth:0}}>
                <div style={{fontWeight:600,fontSize:14,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{c.name}</div>
                <div style={{fontSize:12,color:"#9ca3af",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{c.last?.content}</div>
              </div>
        ...
        {active&&(
          <div style={{background:"var(--card,#fff)",border:"1px solid var(--border,#e8e6e0)",borderRadius:16,display:"flex",flexDirection:"column",height:480}}>
            <div style={{padding:"14px 16px",borderBottom:"1px solid var(--border,#e8e6e0)",display:"flex",alignItems:"center",gap:10}}>
            ...
            <div style={{padding:12,borderTop:"1px solid var(--border,#e8e6e0)",display:"flex",gap:8}}>
              <input className="inp" placeholder="Mesaj yaz..." value={newMsg} onChange={e=>setNewMsg(e.target.value)} onKeyDown={e=>e.key==="Enter"&&sendMsg()} style={{flex:1}}/>
              <button onClick={sendMsg} disabled={sending||!newMsg.trim()} style={{background:"linear-gradient(135deg,#6366f1,#8b5cf6)",color:"#fff",border:"none",borderRadius:12,padding:"0 18px",cursor:"pointer",fontFamily:"'Plus Jakarta Sans'",fontWeight:600,fontSize:13}}>
                {sending?<span className="sp">⟳</span>:t.send}
              </button>
            </div>
          </div>
        )}
      ...
      {modal&&(
        <div className="ovl" onClick={e=>e.target===e.currentTarget&&setModal(false)}>
          <div className="mdl fu">
            <div style={{fontFamily:"'Sora',sans-serif",fontWeight:700,fontSize:22,marginBottom:4}}>{t.newMsg}</div>
            <div style={{fontSize:13,color:"#9ca3af",marginBottom:24}}>Yeni bir konuşma başlat</div>
            <div style={{display:"flex",flexDirection:"column",gap:14}}>
              <div><div className="lbl">ALICI E-POSTA</div><input className="inp" placeholder="ornek@bilkent.edu.tr" value={toEmail} onChange={e=>setToEmail(e.target.value)}/></div>
              <div><div className="lbl">ALICI ADI (opsiyonel)</div><input className="inp" placeholder="Ali Yılmaz" value={toName} onChange={e=>setToName(e.target.value)}/></div>
              <div><div className="lbl">MESAJ</div><textarea className="inp" placeholder="Mesajınız..." rows={4} style={{resize:"vertical"}} value={firstMsg} onChange={e=>setFirstMsg(e.target.value)}/></div>
              <div style={{display:"flex",gap:10}}>
                <button className="gbtn" style={{flex:1}} onClick={()=>setModal(false)}>İptal</button>
                <button className="pbtn" style={{flex:2}} disabled={sending||!toEmail.trim()||!firstMsg.trim()} onClick={startConvo}>{sending?<span className="sp">⟳</span>:"Gönder"}</button>
              </div>
            </div>
          </div>
        </div>
...
function FavoritesPage({user,t}) {
  ...
  return (
    <div className="page">
      <div style={{marginBottom:24}}>
        <div style={{fontFamily:"'Sora',sans-serif",fontWeight:700,fontSize:22}}>{t.favorites}</div>
      </div>
      {loading?(
        <div style={{textAlign:"center",padding:60}}><span className="sp" style={{fontSize:28,color:"#6366f1"}}>⟳</span></div>
      ):posts.length===0?(
        <div style={{textAlign:"center",padding:60,color:"#9ca3af"}}>
          ...
          <div style={{fontSize:14}}>{t.noFav}</div>
        </div>
      ):(
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          {posts.map((p,i)=>{
            ...
            return (
              <div key={p.id} className="card fu" style={{animationDelay:`${i*.04}s`}}>
                ...
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  ...
                  <button onClick={()=>removeFav(p)} style={{background:"none",border:"1px solid #fecaca",borderRadius:100,padding:"4px 12px",color:"#ef4444",cursor:"pointer",fontSize:12,fontFamily:"'Plus Jakarta Sans'"}}>
                    ❤️ Kaldır
                  </button>
                </div>
              </div>
...
function ProfilPage({user,setUser,t}) {
  ...
  async function loadProfile() {
    ...
    if(data){
      setProfile(data)
      setForm({name:data.name||"",bio:data.bio||"",bolum:data.bolum||"",sinif:data.sinif||""})
    } else {
      const fallback={id:user.id,email:user.email,name:user.name,bio:"",bolum:"",sinif:"",avatar_url:null}
      setProfile(fallback)
      setForm({name:user.name,bio:"",bolum:"",sinif:""})
    }
  ...
  return (
    <div className="page">
      <div style={{background:"var(--card,#fff)",border:"1px solid var(--border,#e8e6e0)",borderRadius:20,padding:24,marginBottom:20}}>
        <div style={{display:"flex",alignItems:"flex-start",gap:16,marginBottom:16}}>
          ...
          <div style={{flex:1}}>
            ...
            <div style={{fontSize:12,color:"#6366f1",marginTop:3,fontWeight:500}}>{user.account_type==="sirket"?"🏢 Şirket Hesabı":"🎓 Öğrenci Hesabı"}</div>
          ...
          <button onClick={()=>setEditing(!editing)} style={{background:"none",border:"1px solid var(--border,#e8e6e0)",borderRadius:100,padding:"6px 14px",fontSize:12,color:"#6b7280",cursor:"pointer",fontFamily:"'Plus Jakarta Sans'",fontWeight:500}}>
            {editing?t.cancel:t.edit}
          </button>
        </div>

        {profile.bio&&!editing&&<div style={{fontSize:13,color:"#6b7280",lineHeight:1.6,padding:"12px 0",borderTop:"1px solid var(--border,#e8e6e0)"}}>{profile.bio}</div>}

        {editing&&(
          <div style={{display:"flex",flexDirection:"column",gap:12,borderTop:"1px solid var(--border,#e8e6e0)",paddingTop:16}}>
            ...
            <button className="pbtn" disabled={saving} onClick={saveProfile}>{saving?<span className="sp">⟳</span>:t.save}</button>
          </div>
        )}
      </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10,marginBottom:20}}>
        {[["İlan",myPosts.length],["Beğeni",myPosts.reduce((a,p)=>a+(p.likes||[]).length,0)],["Üye","2025"]].map(([l,v])=>(
          <div key={l} style={{background:"var(--card,#fff)",border:"1px solid var(--border,#e8e6e0)",borderRadius:14,padding:"14px 10px",textAlign:"center"}}>
      ...
      {myPosts.length===0?(
        <div style={{textAlign:"center",padding:40,color:"#9ca3af",background:"var(--card,#fff)",borderRadius:16,border:"1px solid var(--border,#e8e6e0)"}}>
          <div style={{fontSize:32,marginBottom:8}}>📭</div>
...
function SettingsPage({dark,setDark,lang,setLang,t}) {
  return (
    <div className="page">
      <div style={{marginBottom:24}}>
        <div style={{fontFamily:"'Sora',sans-serif",fontWeight:700,fontSize:22}}>{t.settings}</div>
      </div>

      <div style={{display:"flex",flexDirection:"column",gap:12}}>
        <div style={{background:"var(--card,#fff)",border:"1px solid var(--border,#e8e6e0)",borderRadius:16,padding:20}}>
          <div style={{fontFamily:"'Sora',sans-serif",fontWeight:600,fontSize:15,marginBottom:16}}>Görünüm</div>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"12px 0",borderBottom:"1px solid var(--border,#e8e6e0)"}}>
            <div>
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"12px 0"}}>
            ...
            <div style={{display:"flex",gap:8}}>
              {[["tr","🇹🇷 TR"],["en","🇬🇧 EN"]].map(([k,l])=>(
                <button key={k} onClick={()=>setLang(k)} style={{padding:"6px 14px",borderRadius:100,border:`2px solid ${lang===k?"#6366f1":"var(--border,#e8e6e0)"}`,background:lang===k?"#ede9fe":"transparent",color:lang===k?"#6366f1":"#6b7280",cursor:"pointer",fontFamily:"'Plus Jakarta Sans'",fontWeight:600,fontSize:12}}>
                  {l}
                </button>
              ))}
            </div>
        ...
        <div style={{background:"var(--card,#fff)",border:"1px solid var(--border,#e8e6e0)",borderRadius:16,padding:20}}>
          <div style={{fontFamily:"'Sora',sans-serif",fontWeight:600,fontSize:15,marginBottom:16}}>Hakkında</div>
          <div style={{fontSize:13,color:"#9ca3af",lineHeight:1.7}}>
            Kampüs, Bilkent Üniversitesi öğrencileri için geliştirilmiş bir sosyal platform ve ilan ağıdır. Ev arkadaşı bul, eşya sat, staj ilanlarına başvur.
          </div>
          <div style={{marginTop:12,fontSize:12,color:"#c4b5fd",fontWeight:500}}>v2.0.0 — Bilkent Exclusive</div>
        </div>
      </div>
    </div>
  )
...
export default function App() {
    ...
    supabase.auth.getSession().then(async({data:{session}})=>{
      if(session){
        ...
        setUser({
          ...
          email:u.email,
          avatar:profile?.avatar_url||null,
          account_type:profile?.account_type||u.user_metadata?.account_type||"ogrenci"
        })
      }
      setReady(true)
    })
  ...
  const pages={
    feed:<FeedPage user={user} t={t}/>,
    staj:<StajPage user={user} t={t}/>,
    kesfet:<KesfetPage t={t}/>,
    messages:<MessagesPage user={user} t={t} onRead={setUnread}/>,
    favorites:<FavoritesPage user={user} t={t}/>,
  ...
  return (
    <>
      ...
      <div className="layout">
        <Sidebar active={page} setActive={setPage} user={user} onLogout={logout} dark={dark} unread={unread} t={t}/>
        <div className="main-content">{pages[page]}</div>
        <BottomNav active={page} setActive={setPage} unread={unread}/>
      </div>
    </>
  )
}